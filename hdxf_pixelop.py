#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Apply scalar per-pixel arithmetic directly to selected HDXF detector frames.

This is the HDXF counterpart of the original HDF5 pixel-operation script. It
preserves the original arithmetic flag names and selection semantics:

* --add X       : add X to every selected pixel
* --subtract X  : subtract X from every selected pixel
* --multiple X  : multiply every selected pixel by X
* --devide X    : divide every selected pixel by X
* --roi x1,y1,x2,y2 : restrict the arithmetic operation to an inclusive ROI
* --frame_from / --frame_to / --select-frame : global 1-based frame selection

The output is a new self-contained HDXF archive. Non-frame payloads and the
preserved source-HDF5 object graph are copied from the input archive. Selected
frames are decoded, transformed, re-encoded as HDXFB blocks, and described by a
fresh frame index in manifest.json.

Integer arithmetic follows the original HDF5 script:
* add/subtract: clip to dtype range, then cast back to the original dtype;
* multiply/divide: round to nearest integer, clip, then cast back;
* floating-point frames are calculated in float64 and cast back.

With --verify, verification is pixel-exact and block-streaming: source and
output HDXFB blocks are traversed sequentially so each block is decoded at most
once.
...
hdf5_to_hdxf.py supplies the canonical HDXFB encoder used by this project.
"""
...
try:
    ...
    raise SystemExit(
        "HDXF project modules are required. Put hdxf_pixelop.py next to "
...
try:
    ...
    raise SystemExit(
        "hdf5_to_hdxf.py is required next to hdxf_pixelop.py because it "
...
SCRIPT_VERSION = "1.0.0-hdxf-pixelop"
DEBUG = False
...
def _apply_pixel_operation(array: np.ndarray, config: TransformConfig) -> np.ndarray:
    """Apply one scalar arithmetic operation and cast back to the input dtype."""
    ...
    if config.operation == "add":
        if is_int:
            work = array.astype(np.int64) + value
            info = np.iinfo(dtype)
            work = np.clip(work, info.min, info.max)
            return work.astype(dtype, copy=False)
        return (array.astype(np.float64) + value).astype(dtype, copy=False)
    ...
    if config.operation == "multiple":
        if is_int:
            work = np.rint(array.astype(np.float64) * value)
            info = np.iinfo(dtype)
            work = np.clip(work, info.min, info.max)
            return work.astype(dtype, copy=False)
        return (array.astype(np.float64) * value).astype(dtype, copy=False)

    if config.operation == "devide":
        if value == 0.0:
            raise HDXFProcessError("--devide value must be non-zero")
        if is_int:
            work = np.rint(array.astype(np.float64) / value)
            info = np.iinfo(dtype)
            work = np.clip(work, info.min, info.max)
    ...
    raise HDXFProcessError(f"Unknown pixel operation: {config.operation!r}")


def transform_block(block: np.ndarray, config: TransformConfig) -> np.ndarray:
    """Apply the configured arithmetic operation to one (N,H,W) block."""
    source = np.ascontiguousarray(block)
    if source.ndim != 3:
        raise HDXFProcessError(f"Expected a 3D frame block, got shape={source.shape}")

    if config.roi is None:
        return np.ascontiguousarray(_apply_pixel_operation(source, config))

    x0, y0, x1, y1 = config.roi  # x1/y1 are exclusive internally.
    result = source.copy()
    patch = result[:, y0:y1, x0:x1]
    result[:, y0:y1, x0:x1] = _apply_pixel_operation(patch, config)
...
def _parse_roi(
    ...
    *,
    ...
    """Parse original inclusive ROI x1,y1,x2,y2 into exclusive slicing bounds."""
    if not text:
        return None
    try:
        ...
        raise HDXFProcessError(
            "Invalid --roi. Use x1,y1,x2,y2 with 0-based integer coordinates"
        ) from exc
    if len(values) != 4:
        raise HDXFProcessError(
            "Invalid --roi. Use x1,y1,x2,y2 with 0-based integer coordinates"
    ...
    if not (0 <= x1 <= x2 < width and 0 <= y1 <= y2 < height):
        raise HDXFProcessError(
            f"--roi out of bounds for frame shape ({height},{width}): "
            f"{x1},{y1},{x2},{y2}"
        )
    return x1, y1, x2 + 1, y2 + 1


def _selected_operation(args: argparse.Namespace) -> tuple[str, float]:
    operations = [
        ("add", args.add),
    ...
    chosen = [(name, value) for name, value in operations if value is not None]
    if len(chosen) != 1:
        raise HDXFProcessError(
            "Exactly one of --add/--subtract/--multiple/--devide must be provided"
        )
    operation, value = chosen[0]
    assert value is not None
    if operation == "devide" and float(value) == 0.0:
        raise HDXFProcessError("--devide value must be non-zero")
...
def _format_operation_value(value: float) -> str:
...
def _operation_label(
    ...
    *,
    ...
    parts = [f"{tag}_{_format_operation_value(value)}"]
    if roi is not None:
        x0, y0, x1, y1 = roi
        parts.append(f"ROI_{x0},{y0},{x1-1},{y1-1}")
    parts.append(_selection_label(args))
    return "_".join(parts)
...
def _operation_document(
    ...
    *,
    ...
    if roi is not None:
        x0, y0, x1, y1 = roi
        roi_inclusive = [x0, y0, x1 - 1, y1 - 1]
    return {
        "tool": "hdxf_pixelop.py",
        ...
        "roi_inclusive_0based": roi_inclusive,
...
def process_archive(
    ...
    operation, operation_value = _selected_operation(args)
    ...
    if args.threads != 4:
        print(
            "[INFO] --threads is accepted for compatibility with the HDF5 script. "
            "A single HDXF archive is decoded/encoded in order, so this value does "
            "not change intra-archive processing.",
            flush=True,
        )
    ...
    with HDXFArchiveReader(input_path, block_cache_size=2) as reader:
        ...
        roi = _parse_roi(args.roi, width=width, height=height)
        ...
        if not prefix:
            raise HDXFProcessError("--out-prefix may not be empty")
        ...
        if scalar_old_paths:
            dprint(
                f"[DBG] retained {len(scalar_old_paths)} replaced scalar payload member(s)"
            )

        print(f"[START] HDXF pixel operation: {input_path}", flush=True)
        print(
        ...
        print(
            f"[CFG] operation={operation}, value={operation_value:g}",
            flush=True,
        )
        if roi is None:
            print("[ROI] none specified -> whole frame will be processed", flush=True)
        else:
            x0, y0, x1, y1 = roi
            print(
                f"[ROI] using 0-based inclusive coords: TL=({x0},{y0}) "
                f"BR=({x1-1},{y1-1})",
                flush=True,
            )
        print(
    ...
    print("[RESULT] PASS - HDXF pixel operation completed", flush=True)
    return output_path
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Apply one scalar per-pixel arithmetic operation to selected HDXF "
            "detector frames and write a new self-contained HDXF archive."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="1-based inclusive first global frame",
    )
    parser.add_argument(
        "--frame_to",
        "--frame-to",
        ...
        type=int,
        default=None,
        help="1-based inclusive last global frame",
    )
    parser.add_argument(
        "--select-frame",
        type=str,
        default=None,
        help="Comma-separated 1-based global frame indices, e.g. 1,3,2001",
    )

    parser.add_argument("--add", type=float, default=None, help="Add this value to each selected pixel")
    parser.add_argument("--subtract", type=float, default=None, help="Subtract this value from each selected pixel")
    parser.add_argument("--multiple", type=float, default=None, help="Multiply each selected pixel by this value")
    parser.add_argument("--devide", type=float, default=None, help="Divide each selected pixel by this value")
    parser.add_argument(
        "--roi",
        type=str,
        default=None,
        help=(
            "Process only ROI x1,y1,x2,y2 using 0-based inclusive coordinates. "
            "Pixels outside the ROI are copied unchanged."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Frames per output HDXFB block (default: 16)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=4,
        help=(
            "Compatibility with the HDF5 script. One HDXF archive is currently "
            "processed in-order; this value does not change intra-archive workers."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Override output filename prefix (no extension)",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Full pixel-exact verification after writing; uses sequential block "
            "streaming so each HDXFB block is decoded at most once"
        ),
    )
    return parser
